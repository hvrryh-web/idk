...
def create_combatant_from_character(char: Character, techniques: Dict[UUID, TechniqueData]) -> CombatantState:
...
def create_combatant_from_boss(boss: BossTemplate, techniques: Dict[UUID, TechniqueData]) -> CombatantState:
...
def run_simulation(config: SimulationConfig, db: Session) -> Dict:
    ...
    party_chars = db.query(Character).filter(
        Character.id.in_([UUID(cid) for cid in config.party_character_ids])
    ).all()
    ...
    # Load boss template
    boss_template = db.query(BossTemplate).filter(
        BossTemplate.id == config.boss_template_id
    ).first()
    ...
    for trial in range(config.trials):
        ...
        party_states = [
            create_combatant_from_character(char, techniques) 
            for char in party_chars
        ]
